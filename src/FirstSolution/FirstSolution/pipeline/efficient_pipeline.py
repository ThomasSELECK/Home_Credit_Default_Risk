...
def _fit_transform_helper(transformer, X, y, step_index, to_pandas, **fit_params):
    if hasattr(transformer, "fit_transform"):
        X = transformer.fit_transform(X, y, **fit_params)
    else:
        if "y" in list(dict(signature(transformer.transform).parameters).keys()): # If the transformer needs 'y' for training set
...
class EfficientPipeline(Pipeline):
    ...
    def _fit(self, X, y = None, **fit_params):
        ...
        for step_idx, (name, transformer) in enumerate(self.steps[:-1]):
            if transformer is None:
                ...
                if self.cache_dir is not None:
                    cached_steps_paths_dict[name] = fit_transform_helper_cached.get_output_dir(cloned_transformer, Xt, y, step_index, self.to_pandas, **fit_params_steps[name])[0]

                Xt, fitted_transformer = fit_transform_helper_cached(cloned_transformer, Xt, y, step_index, self.to_pandas, **fit_params_steps[name])
    ...
    def fit_transform(self, X, y, **fit_params):
        ...
        if hasattr(last_step, "fit_transform"):
            ...
            if "y" in list(dict(signature(last_step.transform).parameters).keys()): # If the transformer needs 'y' for training set
    ...
    def transform(self, X, y = None):
        ...
        for name, transform in self.steps:
            if transform is not None:
                ...
                if "y" in list(dict(signature(transform.transform).parameters).keys()): # If the transformer needs 'y' for training set
                    Xt = transform.fit(Xt, y, **fit_params).transform(Xt, y)
                else:
