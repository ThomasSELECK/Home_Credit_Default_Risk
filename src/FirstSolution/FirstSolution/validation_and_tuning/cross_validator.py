...
class CrossValidator(object):
    ...
    def __init__(self, evaluation_metrics_lst, monte_carlo_splits = 1, split_type = "random", stratified = True, n_folds = 5, train_window_size = None, test_window_size = None, cv_directory_str = "", n_jobs = -1):
        ...
        split_type : string, either "random" or "time"
        ...
        if type(split_type) != str:
            raise TypeError("The argument 'split_type' MUST be a string, either 'random' or 'time'")

        if split_type != "random" and split_type != "time":
            raise ValueError("The argument 'split_type' MUST be a string, either 'random' or 'time'")
    ...
    def cross_validate(self, modelsPipelines_dict, dataset_df, target_sr):
        """
        ...
        cvType : string. Either "random" or "time"
                This defines the type of cross validation performed depending on the fact that the samples are i.i.d or not.
                When "time" is selected, the argument 'monteCarloSplits' is ignored, its value is fixed to 1.
